            ...
            navigator.geolocation.getCurrentPosition(
                async (position) => {
                    ...
                    try {
                        ...
                        console.log('[useBrowserLocation] OpenCage API response:', response.data);

                        if (response.data.results && response.data.results.length > 0) {
                            const result = response.data.results[0];
                            console.log('[useBrowserLocation] First result:', result);
                            address = {
                            ...
                            console.log('[useBrowserLocation] Parsed address:', address);
                        } else {
                            console.warn('[useBrowserLocation] No results from OpenCage API');
                        }
                    } catch (e: any) {
                        console.error('[useBrowserLocation] Error getting location name:', e.message, e);
                    }
