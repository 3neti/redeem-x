...
const checkStatus = async () => {
    try {
        const response = await fetch(`/redeem/${props.voucher_code}/kyc/status`);
        ...
        if (data.status === 'approved') {
            // Stop polling and redirect to confirm
            ...
            <Card>
                ...
                <CardContent>
                    ...
                    <div v-else-if="status === 'approved'" class="text-center py-8">
                        ...
                        <p class="text-sm text-muted-foreground mt-2">Proceeding to redemption...</p>
                    </div>
